...
def save_results_to_csv(results, project_name,feature_names,model_name):
...
if __name__ == '__main__':
    ...
    column_name = ['commitTime', 'ns', 'nd', 'nf', 'entrophy', 'la', 'ld', 'lt', 'fix', 'ndev', 'age', 'nuc', 'exp',
                   'rexp', 'sexp', 'bug']
    model_names =['logistic_regression','random_forest','naive_bayes','support_vector_machine','gradient_boosting','decision_tree','knn']
    for model_name in model_names:
        for i in range(0, pro_num):
            ...
            results = {key: np.zeros(shape=(0, num_feature)) for key in sampling_methods.keys()}
            #time wise
            gap = 2
            ...
            for fold in range(len(train_folds)):
                ...
                for method, sampler in sampling_methods.items():
                    ...
                    model = tune(train_data, train_label,gap_data, gap_label,model_name)
                    #calculate feature importance
                    perm = PermutationImportance(model).fit(test_data, test_label)
                    result = perm.feature_importances_
                    #merge fold feature importance
                    feature_names = test_data.columns
                    results[method] = np.vstack((results[method], result))
                    print(f"{fold}: {method} is okay~")
            save_results_to_csv(results, project_name,feature_names,model_name)
